#!/bin/bash
# Script para baixar os arquivos listados em cada TXT na pasta plugins_downloads,
# salvando os downloads em subpastas (nome iguais aos TXT) dentro da pasta plugins_downloads_file.
# O script executa até 3 processos aria2c em paralelo e, após cada execução,
# remove os arquivos com tamanho 0 na pasta do plugin.

INPUT_DIR="plugins_downloads"        # Diretório onde estão os arquivos .txt com os links
OUTPUT_BASE="plugins_downloads_file"   # Diretório base onde os downloads serão salvos

echo "🔄 Iniciando o processo de download..."
...
for txtfile in "$INPUT_DIR"/*.txt; do
    ...
    # Inicia a função download_plugin em background
    download_plugin "$txtfile" &
